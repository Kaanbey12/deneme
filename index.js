  ...
  if (message.guild) {
    ...
    if (cmd === "close") {
      ...
      if (message.channel.parentId === category.id) {
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(`Ticket Kapatan Yetkili: **${message.author.tag}**.`);
    ...
    if (cmd === "help") {
      const embed = new MessageEmbed()
        ...
        .setDescription(`This project has been created by Lyrox to let users to make their own modmail bot on their server(s) for free. You can check the support server by clicking [here](https://discord.gg/nodejs).`)
        .addFields(
          {
    ...
    try {
      ...
      if (banCheck || banCheck == 1) {
        ...
        const embed = new MessageEmbed()
          .setDescription(`Ticket Sisteminden Ban Yedin **banned** Sebebi: \`${reason}\``);
      ...
      if (!channel) {
        const ch = await guild.channels.create(message.author.id, {
          type: "text",
          parent: category.id,
          topic: `Yeni Ticket Oluşturuldu Oluşturan; ${message.author.tag} (${message.author.id}) on ${Date.toLocaleString()}.`
        });

        const embedDM = new MessageEmbed()
          .setTitle(`${guild.name} - LÜTFEN BEKLE`)
          .setDescription(`Ticket Başarı İle Oluşturuldu Lütfen Destek Ekibimizden Cevap Bekleyiniz..`)
          .setColor("GREEN");

        message.reply({ embeds: [embedDM], allowedMentions: [{ repliedUser: false }] }).catch(() => { });

        let embedDETAILS = new MessageEmbed()
        .setTitle("Ticket System - Yeni Ticket:")
          .setDescription(`Bir Kullanıcı Ticket Oluşturdu.`)
          .addFields(
            {
              name: "User",
...
client.on('messageCreate', async (message) => {
  ...
  if (message.channel.parentId === category.id) {
    ...
    if (message.member.permissions.has("ADMINISTRATOR")) perms = "Destek Ekibi";
    if (ownerFetch === message.author.id) perms = "Sunucu Sahibi";
...
client.on("channelDelete", async (channel) => {
  ...
  const embedDM = new MessageEmbed()
    ...
    .setDescription(`Ticketiniz bir sunucu Destek Ekibi Rolü tarafından **başarıyla silindi**. Buraya mesaj göndererek yeni bir tane oluşturun.`)
    .setColor("GREEN");
